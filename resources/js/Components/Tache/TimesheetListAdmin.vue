<template>
        ...
        <div v-if="task.timesheets">
            ...
            <div   v-for="timesheet of task.timesheets" :key="timesheet.id" class="w-full text-sm p-3 grid grid-cols-12 border-b cursor-pointer hover:bg-slate-100">
                   <div class="col-span-1 "><span class="bg-green-300 text-green-800 px-1 rounded-full">{{ timesheet.username }}</span></div>
                   <div class="col-span-2">{{ moment(timesheet.date_jour).format('ll') }}</div>
...
import moment from 'moment';
    ...
    const hideMe = ()=> emit('hide')
        ...
        onMounted(() => {
            axios_get('task/timesheet/user/'+props.tache_id).then(({data})=>{
                console.log(data)
                task.value = data
